...
var environment = config['hdfc-cicd-newapi'].env;
...
console.log('&$hdfc-cicd-newapi api: [' +  domains[environment] + ', ' + config['hdfc-cicd-newapi'].basepath + ']');

BeforeAll("BeforeFeatures", function(next) {
    if (apps[0].name) {
        console.log ("Getting Creds from DevAppsKeys.json");
        getCreds(config['hdfc-cicd-newapi'].app, config['hdfc-cicd-newapi'].product);
    } else
    {
...
Before(function(scenario, callback) {
    ...
    this.apickli = new apickli.Apickli(tokens[0],
        tokens[1]+ config['hdfc-cicd-newapi'].basepath,
        './test/integration/features/fixtures/');
    callback();
});
